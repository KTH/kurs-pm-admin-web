...
function* _getUsedRounds(req, res, next) {
  ...
  log.debug(
    "_getUsedRounds with course code: " +
      courseCode +
      "and semester: " +
      semester
  );
...
function* _getKoppsCourseData(req, res, next) {
  ...
  try {
    const apiResponse = yield koppsCourseData.getKoppsCourseData(
      courseCode,
      language
    );
    return httpResponse.json(res, apiResponse.body);
...
function* _saveFileToStorage(req, res, next) {
  ...
  try {
    const fileName = yield runBlobStorage(
      file,
      req.params.semester,
...
function* _updateFileInStorage(req, res, next) {
  log.info(
    "_updateFileInStorage file name:" +
      req.params.fileName +
      ", metadata:" +
      req.body.params.metadata
  );
  try {
    const response = yield updateMetaData(
      req.params.fileName,
      req.body.params.metadata
    );
...
async function getIndex(req, res, next) {
  ...
  try {
    ...
    if (req.params.id.length <= 7) {
      ...
      const apiResponse = await koppsCourseData.getKoppsCourseData(
        req.params.id.toUpperCase(),
        lang
      );
      if (apiResponse.statusCode >= 400) {
        renderProps.props.children.props.routerStore.errorMessage =
          apiResponse.statusMessage; // TODO: ERROR?????
      } else {
    ...
    renderProps.props.children.props.routerStore.__SSR__setCookieHeader(
      req.headers.cookie
    );
