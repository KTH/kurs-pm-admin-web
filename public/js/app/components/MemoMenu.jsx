...
class MemoMenu extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      usedRoundsInWebVer:
        this.props.routerStore.usedRounds.roundsIdWithWebVersion || [],

      temporaryData: this.props.tempData,
      newSemester: false,
      usedRoundSelected: this.props.tempData
        ? this.props.tempData.usedRoundSelected
        : 0,
    };
  ...
  handleRoundCheckbox(event) {
    ...
    if (event.target.checked) {
      ...
      event.target.getAttribute("data-hasfile") === "true"
        ? prevState.usedRoundSelected++
        : prevState.usedRoundSelected;
  ...
  handleCancel() {
    if (
      this.state.temporaryData !== null &&
      this.state.temporaryData.memoFile.length > 0
    ) {
  ...
  getUsedRounds(semester) {
    ...
    return this.props.routerStore
      .getUsedRounds(routerStore.courseData.courseCode, semester)
      .then((result) => {
        thisInstance.setState({
          semester: semester,
          usedRounds: routerStore.usedRounds.usedRoundsIdList || [],
          usedRoundsInWebVer:
            routerStore.usedRounds.roundsIdWithWebVersion || [],
