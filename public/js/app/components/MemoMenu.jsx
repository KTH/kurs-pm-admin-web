...
class MemoMenu extends Component {
  ...
  static getDerivedStateFromProps(props, state) {
    ...
    if (routerStore.usedRounds.usedRoundsIdList || routerStore.hasChangedStatus) {
      const roundsChange = this.getUsedRounds(semester)
      return roundsChange || {}
    } else {
      if (props.progress === 'new_back') {
        return {
          semester,
                          ...
                          <li className="select-list" key={round.roundId}>
                            <Label key={'Label' + round.roundId} for={round.roundId}>
                              <Input
                                ...
                                onChange={(event) => this.handleRoundCheckbox(event)}
                                checked={this.state.rounds.indexOf(round.roundId) > -1}
                                name={round.roundId}
                                disabled={!round.hasAccess}
                                data-hasfile={
                                  this.state.usedRounds.length > 0 &&
                                  this.state.usedRounds.indexOf(round.roundId) > -1
                                }
                              />
                              <RoundLabel
          ...
          <Col sm="12" lg="4">
            {!this.state.firstVisit && !this.state.canOnlyPreview ? (
              <Button
                ...
                id="new"
                key="new"
                onClick={(event) => this.goToEditMode(event)}
